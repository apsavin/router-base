(function (global) {
    ...
    /**
     * @param {{path: String, method: String, host: String|undefined}} request
     * @returns {?Route}
     * @public
     */
    RouterBase.prototype.match = function (request) {
        ...
        for (var i = 0, l = routes.length; i < l; i++) {
            route = routes[i];
            execPathResult = route.pattern.exec(request.path);
            if (!execPathResult) {
                continue;
            }
            execHostResult = route.hostPattern.exec(request.host);
            if (execHostResult && route.methods.indexOf(request.method) > -1) {
                execPathResult.shift();
                execHostResult.shift();
                return {
                    id: route.id,
                    parameters: this._retrieveParameters(route, execPathResult, execHostResult)
                };
            }
        }
        return null;
     ...
     * @private
     */
    RouterBase.prototype._retrieveParameters = function (route, parsedParameters, parsedHostParameters) {
        var parameters = {};
    ...
    RouterBase.prototype._generate = function (route, params) {
        ...
        return generatedHost ? '//' + generatedHost + generatedPath : generatedPath;
    };
